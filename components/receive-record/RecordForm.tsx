...
const RecordForm: React.FC<RecordFormProps> = ({ onSave, wards, records, holidays, calculateDeadline, generateCode, onPrint, initialData, onCancelEdit, currentUser, employees, currentView, onLoadingChange, isInModal, onReceiptNumberChange, resetTrigger }) => {
  ...
  const addOtherDocRow = () => {
      setOtherDocRows(prev => [...prev, { name: '', type: 'Bản chính' }]);
  };
  ...
  const plainInputClass = "w-full px-3 py-2 border border-slate-300 rounded-md text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 transition-all font-medium text-slate-700 bg-white hover:border-slate-400";
  const selectClass = "w-full px-3 py-2 border border-slate-300 rounded-md text-sm bg-white outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 transition-all font-medium text-slate-700 cursor-pointer hover:border-slate-400";
  const labelClass = "block text-xs font-bold text-slate-700 mb-1.5 ml-0.5";
        ...
        <div className="flex flex-col gap-6">
                    ...
                    <div className="p-4 space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                            <div className="md:col-span-1">
                                ...
                                <input 
                                    type="text" 
                                    className={`${plainInputClass} bg-white text-slate-800 font-bold font-mono`} 
                            ...
                            {isRegistration(formData.recordType) && !isDefaultTaxProcedure(formData.recordType) && (
                                <div className="md:col-span-1 flex items-center h-[38px] pb-1 pl-2">
                                    <label className="flex items-center gap-2 text-sm font-bold text-slate-700 cursor-pointer">
                                        <input
                                            type="checkbox"
                                            className="w-4 h-4 text-blue-600 border-slate-300 rounded focus:ring-blue-500"
                                            checked={hasTaxProcedure}
                                            onChange={(e) => setHasTaxProcedure(e.target.checked)}
                                        />
                                        Có quy trình thuế
                                    </label>
                                </div>
                        ...
                        <table className="min-w-full divide-y divide-slate-200 text-sm">
                            <thead className="bg-slate-50">
                                <tr>
                                    ...
                                    <th className="px-3 py-2 text-center text-xs font-bold text-slate-500 uppercase w-60">Hình thức nộp</th>
                                    <th className="px-3 py-2 text-center text-xs font-bold text-slate-500 uppercase w-16">Xóa</th>
                                </tr>
                            </thead>
                            <tbody className="bg-white divide-y divide-slate-200">
                                {otherDocRows.map((row, index) => (
                                    <tr key={index}>
                                        ...
                                        <td className="px-3 py-1.5">
                                            <input 
                                                type="text" 
                                                className="w-full border border-slate-300 rounded px-2 py-1 text-sm bg-white focus:border-blue-500 outline-none" 
                                                value={row.name} 
                    ...
                    <div className="p-4 space-y-4 animate-fade-in">
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                            <div>
                                ...
                                <input type="text" placeholder="Họ tên người được ủy quyền..." className={plainInputClass} value={formData.authorizedBy || ''} onChange={(e) => handleChange('authorizedBy', e.target.value)} />
                            </div>
                            <div>
                                <label className={labelClass}>Số giấy tờ tùy thân</label>
                                <input type="text" placeholder="Số CCCD/CMND..." className={plainInputClass} value={authDocNumber} onChange={(e) => setAuthDocNumber(e.target.value)} />
                            </div>
                            <div>
                                <label className={labelClass}>Loại giấy tờ ủy quyền</label>
                                <select className={selectClass} value={(formData.authDocType || '').split('|')[0] || ''} onChange={(e) => handleChange('authDocType', e.target.value ? `${e.target.value}|${(formData.authDocType || '').split('|')[1] || 'Bản chính'}` : '')}>
                            ...
                            <div>
                                ...
                                <div className="flex items-center gap-6 h-[38px] px-2">
                                    <label className="flex items-center gap-1.5 text-sm cursor-pointer font-semibold text-slate-700">
                                        <input type="radio" name="authDocCopy" value="Bản chính" checked={((formData.authDocType || '').split('|')[1] || 'Bản chính') === 'Bản chính'} onChange={(e) => handleChange('authDocType', (formData.authDocType || '').split('|')[0] ? `${(formData.authDocType || '').split('|')[0]}|${e.target.value}` : '')} className="text-blue-600 focus:ring-blue-500" />
                                        Bản chính
                                    </label>
                                    <label className="flex items-center gap-1.5 text-sm cursor-pointer font-semibold text-slate-700">
                                        <input type="radio" name="authDocCopy" value="Bản sao" checked={((formData.authDocType || '').split('|')[1] || 'Bản chính') === 'Bản sao'} onChange={(e) => handleChange('authDocType', (formData.authDocType || '').split('|')[0] ? `${(formData.authDocType || '').split('|')[0]}|${e.target.value}` : '')} className="text-blue-600 focus:ring-blue-500" />
                                        Bản sao
                                    </label>
                                </div>
                            </div>
            ...
            {initialData && initialData.id && (
                <div className="space-y-6 mt-4">
                    {/* SECTION: SỐ TRÍCH ĐO, SỐ TRÍCH LỤC, NHÂN VIÊN */}
                    <div className="bg-white rounded-lg border border-slate-200 shadow-sm p-4">
                        <div className={`grid grid-cols-1 ${isMeasurement ? 'md:grid-cols-3' : 'md:grid-cols-1'} gap-4`}>
                            {isMeasurement && (
                                <>
                                    <div>
