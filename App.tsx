  ...
  const advanceStatus = useCallback(async (record: RecordFile) => {
      ...
      if (isRegType(record.recordType)) {
          ...
          if (nextIdx < stepConfigs.length) {
              ...
              if (nextStep.label.includes("In GCN") || nextStep.label.includes("In Giấy chứng nhận")) {
                  ...
                  <div className="fixed inset-0 bg-black/60 z-[9999] flex items-center justify-center p-4 backdrop-blur-sm">
                      <div className="bg-white rounded-xl shadow-2xl border border-indigo-100 w-full max-w-md overflow-hidden animate-fade-in-up text-left">
                          <div className="bg-indigo-600 px-5 py-3 text-white font-bold text-sm flex items-center justify-between">
                               <span>CẬP NHẬT THÔNG TIN THỬA ĐẤT MỚI</span>
                          ...
                          <div className="p-5 space-y-4">
                              <p className="text-xs text-gray-600 leading-relaxed">
                                  Vui lòng nhập thông tin thửa đất mới, tờ bản đồ và diện tích cho hồ sơ <strong>{taxTargetRecord.code}</strong> trước khi lập phiếu chuyển thuế:
                              </p>
                              ...
                              <div>
                                  <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Diện tích mới (m²)</label>
                                  <input 
                                      type="number" 
                                      value={taxArea} 
                                  ...
                                  <button
                                      onClick={async () => {
                                          ...
                                          if (currentStepIndex === undefined || currentStepIndex === null) {
                                              const foundIdx = stepConfigs.findIndex(s => s.label.includes("Phiếu chuyển Thuế") || s.label.includes("Lập phiếu chuyển thuế"));
